...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_tabs__WEBPACK_IMPORTED_MODULE_2__["default"])('.decoration_slider', '.no_click', '.decoration_content > div > div', 'after_click'); //ТЗ 7
    ...
    elem.forEach(function (item, i) {
      item.addEventListener(event, function () {
        switch (item.nodeName) {
          //nodeName -название строки каждого эл, если оно повдаете с сase
          case 'SPAN':
            state[prop] = i;
            break;
...
var forms = function forms(state) {
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      var formData = new FormData(item); //FormData это объект, ктр соберет все содержание в инпутах и поместить в перемен formData
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  var header = document.querySelector(headerSelector),
      ...
      content = document.querySelectorAll(contentSelector); //функция, ктр будет скрывать все табы
  ...
  hideTabContent();
  showTabContent(); //Отслеживаем какой таб кликнул пользователь, чтобы запустить фун-ции. Для этого используем делегирование событий
...
var timer = function timer(id, deadline) {
  ...
  function setClock(selector, endtime) {
    ...
    updateClock(); //функция ктр будет обновлять таймер каждую секунду
