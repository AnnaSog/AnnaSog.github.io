...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_pictureSize__WEBPACK_IMPORTED_MODULE_8__["default"])('.sizes-block'); // accordion('.accordion-heading','.accordion-block'); //I вариант

  Object(_modules_accordion__WEBPACK_IMPORTED_MODULE_9__["default"])('.accordion-heading'); //II вариант
...
var accordion = function accordion(triggersSelector) {
  ...
  btns.forEach(function (btn) {
    btn.addEventListener('click', function () {
      btns.forEach(function (btn) {
        //весь контент скрыт, будет работать только действующая кнопка с контентом, все ост будут закрыты
...
/* harmony default export */ __webpack_exports__["default"] = (accordion); // // I ВАРИАНТ реализации аккордеона с помощья сss-анимации
...
var burger = function burger(burgerSelector, menuSelector) {
  ...
  burgerElem.addEventListener('click', function () {
    if (menuElem.style.display == 'none' && window.screen.availWidth < 993) {
      //если меню скрыто и ширина всего экрана(availWidth) меньше 993
  ...
  window.addEventListener('resize', function () {
    if (window.screen.availWidth > 992) {
      //если размер экрана больше 992, то меню скрываем
      menuElem.style.display = 'none';
    }
  });
};
...
var checkTextInputs = function checkTextInputs(selector) {
  //заполнение имени и комм только на рус яз
  var txtInputs = document.querySelectorAll(selector);
  txtInputs.forEach(function (input) {
    input.addEventListener('keypress', function (e) {
      //'keypress' - польз.нажимает на определен. клавишу
      if (e.key.match(/[^а-яё 0-9]/ig)) {
        e.preventDefault();
      }
    });
    input.addEventListener('input', function () {
      //если польз.внес сохраненные данные на латиницы, то они очищаются
...
var drop = function drop() {
  var fileInputs = document.querySelectorAll('[name="upload"]'); // dragenter - объект над dropArea(любой эл., ктр воспринимает это событие)
  ...
  ['dragenter', 'dragleave', 'dragover', 'drop'].forEach(function (eventName) {
    //в [] перечислены обработчики событий для реализации Drag&Drop
  ...
  ['dragenter', 'dragover'].forEach(function (eventName) {
    //эти 2 события вызывают подсветку, далее на каждый input вешаем событие и подставляем highlight
    fileInputs.forEach(function (input) {
  ...
  ['dragleave', 'drop'].forEach(function (eventName) {
    //эти 2 события убирают подсветку
    fileInputs.forEach(function (input) {
      input.addEventListener(eventName, function () {
  ...
  fileInputs.forEach(function (input) {
    input.addEventListener('drop', function (e) {
      ...
      var dots; //будет содержать ... или .

      var arr = input.files[0].name.split('.'); //разбивает назв 1-ого файла до . 
...
var filter = function filter() {
  var menu = document.querySelector('.portfolio-menu'),
      ...
      no = document.querySelector('.portfolio-no'); //фун-ия фильтрации эл. по типу, вскрытие ненужных, показ нужных

  var typeFilter = function typeFilter(markType) {
    markAll.forEach(function (mark) {
      //перебираем все эл.на гл обертке(wrapper),где все изображения и скрываем их
    ...
    if (markType) {
      //если контект заполнен,т.е. нажата кнопка и в этом блоке есть изображения, то их показ
  ...
  menu.addEventListener('click', function (e) {
    //при делегирован.необходимо е - event
    var target = e.target; //эл. на ктр происходит событие

    if (target && target.tagName === 'LI') {
      //если эл. поддерживает событие и имя события LI
      items.forEach(function (btn) {
        return btn.classList.remove('active');
      }); //то у всех кн меню удаляем класс актив  
...
var forms = function forms() {
  ...
  var path = {
    //пути по ктр будем отправлять данные
  ...
  upload.forEach(function (item) {
    item.addEventListener('input', function () {
      //как только польз загружил файл
      console.log(item.files[0]); //если длинное название, необходимо обрезать

      var dots; //будет содержать ... или .

      var arr = item.files[0].name.split('.'); //разбивает назв 1-ого файла до . 
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      var statusImg = document.createElement('img'); //img ктр будет всплывает после отправ формы

      statusImg.setAttribute('src', message.spinner);
      statusImg.classList.add('animated', 'fadeInUp');
      statusMessage.appendChild(statusImg);
      var textMessage = document.createElement('div'); //добавим к изобр текстовое сообщение

      textMessage.textContent = message.loading;
      statusMessage.appendChild(textMessage); //FormData это объект, ктр соберет все содержание в инпутах и помещает в перемен formData

      var formData = new FormData(item);
      var api; //динамический путь куда все будет отпр

      item.closest('.popup-design') || item.classList.contains('calc_form') ? api = path.designer : api = path.question;
      console.log(api); //отправляем переменую postData на сервер 
...
var mask = function mask(selector) {
  ...
  function createMask(event) {
    var matrix = '+7 (___) ___ __ __',
        ...
        //все не цифры в матрице наменяет на ""
    val = this.value.replace(/\D/g, ''); //все не цифры внесенные пользователем наменяет на ""

    if (def.length >= val.length) {
      //если кол-во цифр в матрице >=  кол-ву цифр внесенных польз.
      val = def;
    }

    this.value = matrix.replace(/./g, function (a) {
      //пройдемся по всем симв эл и на каждого сработает функ
    ...
    if (event.type === 'blur') {
      //если польз перестал вносить в инпут
      if (this.value.length == 2) {
        //если кол-во цифр внесен.польз =2
        this.value = '';
      }
    } else {
  ...
  var inputs = document.querySelectorAll(selector); //на этот эл использ маску
...
var modals = function modals() {
  var btnPressed = false; //ни одна кнопка не нажата

  function bindModals(triggerSelector, modalSelector, closeSelector) {
    var destloy = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
    ...
    trigger.forEach(function (item) {
      item.addEventListener('click', function (e) {
        ...
        if (destloy) {
          //если равен true, то удаляем этот эл
          item.remove();
        }
        ...
        document.body.style.marginRight = "".concat(scroll, "px"); //добавится отступ 
      });
    });
  ...
  function showModalbyTime(selector, time) {
    setTimeout(function () {
      ...
      document.querySelectorAll('[data-modal]').forEach(function (item) {
        //получаем доступ по всем мод окнам
        if (getComputedStyle(item).display !== 'none') {
          //если перебираемое мод окно открыто
          display = 'block';
        }
      });

      if (!display) {
        //если ни одно мод окно не открыто, то всплывает нужное мод окно через время
  ...
  function openByScroll(selector) {
    window.addEventListener('scroll', function () {
      var scrollHeight = Math.max(document.documentElement.scrollHeight, document.body.scrollHeight); //этот код испол.для старых браузеров. Библиот.Math вернет max знач. ктр указано в арг
...
var pictureSize = function pictureSize(imgSelector) {
  var blocks = document.querySelectorAll(imgSelector); //фун-ия принятия блока и показа
  ...
  function showImg(block) {
    ...
    block.querySelectorAll('p:not(.sizes-hit)').forEach(function (p) {
      //все р-параграфы, кроме .sizes-hit(Хит продаж), будут скрыты
  ...
  function hideImg(block) {
    ...
    block.querySelectorAll('p').forEach(function (p) {
      //все р-параграфы будут показаны
...
var scrolling = function scrolling(upSelector) {
  ...
  window.addEventListener('scroll', function () {
    if (document.documentElement.scrollTop > 1650) {
      //если прокручено 1650, то up плавно появляется            
      upElem.classList.add('animated', 'fadeIn');
      upElem.classList.remove('fadeOut');
    } else {
      //если меньше 1650, то up плавно исчезает
      upElem.classList.add('fadeOut');
      upElem.classList.remove('fadeIn');
    }
  }); //Реализация плавного скролла
  //II ВАРИАНТ - с помощью requestAnimationFrame (в этом варианте также будет реализация плавного скролла ссылок в меню)

  var links = document.querySelectorAll('[href^="#"]'),
      //доступ по всем ссылкам, ктр начинаются с #
  speed = 0.7;
  links.forEach(function (link) {
    link.addEventListener('click', function (event) {
      ...
      var heightTop = document.documentElement.scrollTop,
          //доступ к эл., ктр показывает пролистаное
      hash = this.hash,
          //текущий #
      toBlock = document.querySelector(hash).getBoundingClientRect().top,
          //getBoundingClientRect-метод позвол.получить доступ к св-ву top и возвр.его размер
      start = null; //стартовая позиция пока null

      requestAnimationFrame(step);

      function step(time) {
        //аргумент time передается по умолчанию
        if (start === null) {
          //если анимация запускается первый раз   
          start = time;
        } //это условие выполняется 1 раз, тк в сл. раз в start будет уже time


        var progress = time - start,
            //time будет каждый раз приходить новое время, start - только вначале
        ...
        if (r != heightTop + toBlock) {
          //если не пролистано до нужно рх        
  ...
  }); // // I ВАРИАНТ- чистое исполнение js
  // const element = document.documentElement,
  //     body = document.body;
  // //фун-ия занимается подсчетом, сколько надо пролистать и как это сделать 
  // const calcScroll = () =>{
  ...
  //             let hashElement = document.querySelector(this.hash),        //эл. к ктр будем скроллить стр (this.hash - эл к ктр листаем)
  //                hashElementTop = 0;                                      //сколько еще нужно пролистать рх до родителя hashElement
  //             while(hashElement.offsetParent){                            //пока родитель hashElement будет существовать, будет запускаться цикл
  //                 hashElementTop += hashElement.offsetParent;             //ск. рх ост. до родителя верхней границы от hashElement 
  ...
  //         if(
  //             (prevScrollTop === scrollTop ||                               //если предыдущее значение = пролистаному или
...
var sliders = function sliders(slides, dir, prev, next) {
  //dir -направление слайдов
  var slideIndex = 1,
      //нумерация слайда
  paused = false; //останавливает автоматическое переключение
  ...
  function showSlides(n) {
    ...
    items.forEach(function (item) {
      item.classList.add('animated'); //для использ. animate.css (для плавного перехода слайда)
  ...
  showSlides(slideIndex); //фун-ия переключение слайдов
